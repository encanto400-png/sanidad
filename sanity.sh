...
if [ "$ALL_UP" = "true" ]; then
    ...
    # Enviar email cada cierto tiempo, independientemente de cada cuanto se ejecute la tarea
    ...
    if $send_email; then
        ...
        # Validar variable MAILTO (To)
        if [ -z "${MAILTO}" ]; then
            log_message "ERROR" "MAILTO is empty, cannot send email"
            exit 1
        fi

        # Validar variable MAILFROM (From)
        if [ -z "${MAILFROM}" ]; then
            # Si no viene en el cfg, usar el FID como remitente
        ...
        # Determinar si se incluye CC_LIST en el correo
        CC_HEADER=""
        if [ -n "${CC_LIST}" ]; then
            CC_HEADER="Cc: ${CC_LIST}"
        fi

        # Enviar notificación por correo si alguna tarea falla
